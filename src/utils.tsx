    ...
    while(!isPuzzle) {
        ...
        for(let i = 0; i < 9; i += 1) {
            ...
            for(let j = 0; j < 9; j  += 1) {
                ...
                const choices = [...rows[i]].filter(x => cols[j].includes(x)).filter(x => squares[sqIdx].includes(x));
...
export const pluck = (puzzle: number[][], n = 0) => {
    ...
    while(cells.length > n && count < n) {
        ...
        let checkRow = false, checkCol = false, checkSquare = false;
        for(let i = 0; i < 9; i += 1) {
            const row = { x: i, y };
            const col = { x, y: i };
            const square = {
            ...
            if(row.y !== y) {
                checkRow = canBe(puzzle, row.x, row.y, cell);
            }
            if(col.x !== x) {
                checkCol = canBe(puzzle, col.x, col.y, cell);
            }
            if(square.x !== x && square.y !== y) {
                checkSquare = canBe(puzzle, square.x, square.y, cell);
            }
        }
